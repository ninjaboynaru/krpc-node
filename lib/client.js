...
function decodeObject(client, decoder, data) {
    ...
    Object.keys(map).forEach(propertyName => {
        ...
        if (typeof value === 'string') {
            object[propertyName] = injectObjectIdIntoChildFunction(service['_' + value], object.id);
            return;
        }
		...
		let getterFunctionName = value['get'];
		let setterFunctionName = value['set'];

		// Why check for both -  some classes only implement get or set
		if(typeof getterFunctionName !== 'string' && typeof setterFunctionName !== 'string') {
			throw new Error(`Incorrect map for ${propertyName}, ${propertyName} object should have get and set properties set to strings`);
		}


		if(getterFunctionName) {
			const getFunction = injectObjectIdIntoChildFunction(service['_' + getterFunctionName], object.id);
		...
		if(setterFunctionName) {
			const setFunction = injectObjectIdIntoChildFunction(service['_' + setterFunctionName], object.id);
...
function injectObjectIdIntoChildFunction(childFunction, id) {
    return async function() {
        Array.prototype.unshift.call(arguments, id);
        return childFunction.apply(childFunction, arguments);
    };
}
...
function addDirectInvoker(service, client) {
    ...
    Object.keys(service).forEach(key => {
        ...
        wrappedService['_' + key] = async function() {
            ...
            const response = await client.send(procedure);

			if(response.error) {
				throw new Error(`${'_' + key} response contains an error: ${response.error.description}`);
			}
			else {
